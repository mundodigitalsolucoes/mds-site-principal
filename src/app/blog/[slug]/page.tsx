    ...
    <main id="topo" className="bg-white text-slate-900 selection:bg-[#374b89] selection:text-white">
      ...
      <section className="border-b border-slate-200 bg-gradient-to-b from-slate-50 to-white">
        <div className="mx-auto max-w-4xl px-6 py-12 md:px-8 md:py-16">
          <nav className="mb-6 flex flex-wrap items-center gap-2 text-sm text-slate-500">
            <Link href="/" className="hover:text-[#374b89]">Início</Link>
            <span>/</span>
            <Link href="/blog" className="hover:text-[#374b89]">Blog</Link>
            <span>/</span>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-slate-500">
            <div className="flex items-center gap-3">
              {post.authorImage && (
                <Image src={post.authorImage} alt={post.author} width={44} height={44} className="h-11 w-11 rounded-full object-cover ring-2 ring-[#374b89]/10" />
              )}
              <div className="flex flex-col leading-tight">
                <span className="font-semibold text-[#2f3453]">{post.author}</span>
                <span className="text-xs text-slate-500">Especialista em marketing e vendas</span>
              </div>
            </div>
          ...
          <div className="my-10 rounded-3xl bg-[#2f3453] px-8 py-10 text-white">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              {showSecondaryCta && post.cta.secondaryLabel && post.cta.secondaryHref && (
                <Link href={post.cta.secondaryHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-xl border border-white/20 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/10">{post.cta.secondaryLabel}</Link>
              )}
            </div>
          </div>
        )}
        ...
        <section id="contato" className="mx-auto max-w-4xl px-6 py-8 md:px-8">
          <div className="rounded-3xl border border-slate-200 bg-slate-50 px-8 py-10">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              {showSecondaryCta && post.cta.secondaryLabel && post.cta.secondaryHref && (
                <Link href={post.cta.secondaryHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:border-[#374b89] hover:text-[#374b89]">{post.cta.secondaryLabel}</Link>
              )}
            </div>
          </div>
        </section>
      ...
      {relatedPosts.length > 0 && (
        <section className="mx-auto max-w-7xl px-6 py-12 md:px-8">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-[#2f3453]">Continue lendo</h2>
            <p className="mt-2 text-slate-600">Mais conteúdos da mesma linha para aprofundar o tema.</p>
          </div>
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {relatedPosts.map((relatedPost) => (
              <article key={relatedPost.slug} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
                <Link href={`/blog/${relatedPost.slug}`} className="relative block h-56 bg-slate-100">
                  <Image src={relatedPost.coverImage} alt={relatedPost.title} fill className="object-cover" />
                </Link>
                <div className="p-6">
                  <span className="rounded-full bg-[#374b89]/10 px-3 py-1 text-sm font-semibold text-[#374b89]">{relatedPost.category}</span>
                  <h3 className="mt-4 text-xl font-bold leading-snug text-[#2f3453]">
                    <Link href={`/blog/${relatedPost.slug}`} className="hover:text-[#374b89]">{relatedPost.title}</Link>
                  </h3>
                  <p className="mt-3 text-sm leading-6 text-slate-600">{relatedPost.excerpt}</p>
                  <div className="mt-5">
                    <Link href={`/blog/${relatedPost.slug}`} className="text-sm font-semibold text-[#374b89] hover:text-[#2f3453]">Ler artigo</Link>
                  </div>
                </div>
              </article>
            ))}
